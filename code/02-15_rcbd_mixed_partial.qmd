...
# Model fitting
rcbd_mix_mod <- lmer(yield_kgha ~ nrate_kgha * krate_kgha + (1|rep), data = rcbd_dfw)
...
# Model fitting
rcbd_mix_mod_fix <- lm(yield_kgha ~ nrate_kgha * krate_kgha + rep, data = rcbd_dfw)

# Summary
summary(rcbd_mix_mod_fix)
```

```{r}
Anova(rcbd_mix_mod_fix, type = 3)
```
