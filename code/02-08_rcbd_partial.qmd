...
# Model fitting
rcbd_mod <- lm(yield_kgha ~ nrate_kgha * krate_kgha + rep, data = rcbd_dfw) # rep is never added as interaction in rcbd design when it is ran as a fixed effect
...
ggplot(mapping = aes(fill = trtname))+
  geom_boxplot(data = rcbd_dfw, aes(x = trtname, y = yield_kgha), alpha = 0.8, show.legend = F)+
  geom_jitter(data = rcbd_dfw, aes(x = trtname, y = yield_kgha), shape = 21, show.legend = F, size = 3, alpha = 0.5)+
  geom_label(data = rcbd_cld_selected, aes(label = letters, y = emmean, x = trtname), fill = "white", show.legend = F)+
